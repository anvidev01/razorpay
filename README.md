...
| **Build quality** — does it run, is it structured, would you trust it | [`./verify.sh`](#build-quality--one-command-proves-every-claim) — 48 checks, exits non-zero if any claim here is false |
...
./verify.sh            # 48 checks, ~90s   (--quick skips benchmarks/sanitizers, ~15s)
```
